const app = function() {
    ...
    const loadPage = async function() {
        ...
        try {
            ...
            if (!currentUser ) {
                window.location.replace("http://localhost:8080/maven-banking-app/login.jsp");
            } else {
                ...
                roleEl.innerText = `Role: ${currentUser.role}`;
    ...
    const loadUserInfo = async function(event, user) {
        ...
        try {
            ...
            console.error("Error loading statuses or types:", error);
        }
    };
    ...
    const loadAccountInfo = async function(event, account) {
        ...
        try {
            ...
            const accountInfoEditBtn = document.create;
    ...
    const handleDeleteAccount = async function(event, account) {
        event.preventDefault();
        try {
            await axios.delete(`/accounts/${account.accountId}`);
        } catch (err) {
    ...
    const handleDeposit = async function(event, accountId, amount) {
        event.preventDefault();
        try {
            await axios.post("/accounts/deposit", {
                accountId,
                amount
            });
        } catch (err) {
    ...
    const handleWithdraw = async function(event, accountId, amount) {
        event.preventDefault();
        try {
            await axios.post("/accounts/withdraw", {
                accountId,
                amount
            });
        } catch (err) {
    ...
    const passTime = async function(event) {
        event.preventDefault();
        try {
            await axios.post("/passTime", {
                numOfMonths: passTimeEl.value
            });
        } catch (err) {
            console.error("Error passing time:", err);
        }
        loadPage();
    };
